...
upload-dropbox:
	docker run --rm  -e DROPBOX_TOKEN=$(DROPBOX_TOKEN) -v "$(HOST):/app" "$(PIPENV_IMAGE)" sh -c "pipenv install && pipenv run python upload_dropbox.py --source=Resume/Resume.pdf --target=/Resume.pdf"
